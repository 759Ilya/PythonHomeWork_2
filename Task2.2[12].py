...
# Для вычисления квадратного корня используйте возведение в степень 0.5 или (*) Усложнение. найдите самостоятельно в сети какая функция стандартной библиотеки вычисляет 
# квадратный корень и как до нее добраться.

from math import sqrt
s, p = map( int, input('Введите через пробел сумму чисел-s и произведение этих чисел-p : ').split() )
z = sqrt( (s/2)**2 - p )
print( f'Задуманные Петей числа:', int( s/2 - z ), int( s/2 + z ) )